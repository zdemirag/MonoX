...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("ttbar_80x_full.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("ttbar_80x_full.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("ttbar_80x_full.root:/nero");
